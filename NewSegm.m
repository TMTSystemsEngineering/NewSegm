...
%     Corrected incorrect rotation direction for 11D
%
...
for i=1:n_segments
    ...
    % Noll Zernike coefficients rotated into PSA coordinates
    m = 0;
    PSANollZernike(i).c20 = NollZernike(i).c20;
    PSANollZernike(i).c40 = NollZernike(i).c40;
    
    m = 1;
    R11 = cos(m*angle);
    R21 = sin(m*angle);
    PSANollZernike(i).c31 = R11 * NollZernike(i).c31;
    PSANollZernike(i).c3m1 = R21 * NollZernike(i).c31;  
    
    m = 2;
    R11 = cos(m*angle);
    R21 = sin(m*angle);
    PSANollZernike(i).c22 = R11 * NollZernike(i).c22;
    PSANollZernike(i).c2m2 = R21 * NollZernike(i).c22;  
    PSANollZernike(i).c42 = R11 * NollZernike(i).c42;
    PSANollZernike(i).c4m2 = R21 * NollZernike(i).c42; 
    
    m = 3;
    R11 = cos(m*angle);
    R21 = sin(m*angle);
    PSANollZernike(i).c33 = R11 * NollZernike(i).c33;
    PSANollZernike(i).c3m3 = R21 * NollZernike(i).c33;  
...
fprintf(fid,'TMT M1 SEGMENTATION DATA - TMT.OPT.TEC.07.044.CCR16 - %s\n',datestr(now));
...
fprintf(fid,' \n');
fprintf(fid,'11E: NOLL ZERNIKE COEFFICIENTS ROTATED INTO THE PSACRS AXES (microns)\n');
fprintf(fid,'type#\t   radius (m)\t          C20\t         C2+2\t         c2-2\t         C3+1\t         C3-1\t         C3+3\t         C3-3\t         C40\t         C4+2\t         C4-2\t \n');
for i=1:n_segments    
    fprintf(fid,'%3i\t%13.9f\t%13.6f\t%13.6f\t%13.6f\t%13.6f\t%13.6f\t%13.6f\t%13.9f\t%13.9f\t%13.9f\t%13.9f\n',i,prescriptionRadius(i), 1e6*PSANollZernike(i).c20, 1e6*PSANollZernike(i).c22, 1e6*PSANollZernike(i).c2m2, 1e6*PSANollZernike(i).c31, 1e6*PSANollZernike(i).c3m1, 1e6*PSANollZernike(i).c33, 1e6*PSANollZernike(i).c3m3, 1e6*PSANollZernike(i).c40, 1e6*PSANollZernike(i).c42, 1e6*PSANollZernike(i).c4m2);
end;
fprintf(fid,' \n');
